...
class CodeGraph:
    ...
    def store_document(
        ...
        """Store a documentation file.

        Args:
            file_path: Document file path
            content: Document content
            doc_type: Document type (readme, api, guide, contributing, etc.)
            title: Optional document title
            format: Document format (markdown, rst, txt, etc.)
            metadata: Optional additional metadata
        ...
        self._portal._db.execute_command(f"""
            INSERT INTO _ci_documents (
                document_id, project_id, file_path, doc_type, title,
                content, content_hash, format, word_count, metadata
            ) VALUES (
                '{document_id}',
                '{proj_id}',
                '{_escape(file_path)}',
        ...
        logger.info(f"Stored document {file_path} ({word_count} words)")
        return document_id

    def get_document(self, file_path: str, project_id: str | None = None) -> dict[str, Any] | None:
        ...
        if proj_id:
            return self._portal._db.query_single(
                f"SELECT * FROM _ci_documents "
                f"WHERE project_id = '{proj_id}' AND file_path = '{_escape(file_path)}' "
                "ORDER BY indexed_at DESC LIMIT 1"
            )
        else:
            return self._portal._db.query_single(
                f"SELECT * FROM _ci_documents WHERE file_path = '{_escape(file_path)}' "
                "ORDER BY indexed_at DESC LIMIT 1"
            )
    ...
    def list_documents(
        ...
        Args:
            project_id: Optional project ID filter (uses current if not provided)
        ...
        conditions = []
        if proj_id:
            conditions.append(f"project_id = '{proj_id}'")
        if doc_type:
        ...
        return self._portal._db.query(
            f"SELECT document_id, project_id, file_path, doc_type, title, "
            f"word_count, format, indexed_at FROM _ci_documents {where} "
            "ORDER BY file_path"
        )
    ...
    def search_documents(
        ...
        conditions = [f"content LIKE '%{_escape(query)}%'"]
        if proj_id:
            conditions.append(f"project_id = '{proj_id}'")
        ...
        return self._portal._db.query(
            f"SELECT document_id, project_id, file_path, doc_type, title, "
            f"word_count FROM _ci_documents {where} "
            f"ORDER BY word_count DESC LIMIT {limit}"
        )

    def delete_document(self, file_path: str, project_id: str | None = None) -> None:
        """Delete a document.
        ...
        if proj_id:
            self._portal._db.execute_command(
                f"ALTER TABLE _ci_documents DELETE "
                f"WHERE project_id = '{proj_id}' AND file_path = '{_escape(file_path)}'"
            )
        else:
            self._portal._db.execute_command(
                f"ALTER TABLE _ci_documents DELETE WHERE file_path = '{_escape(file_path)}'"
            )
    ...
    def create_session(
        ...
        Args:
            title: Session title
            project_id: Project ID (uses current if not provided)
        ...
        self._portal._db.execute_command(f"""
            INSERT INTO _ci_sessions (
                session_id, project_id, title, description, model, provider,
                ...
                {f"'{_escape(model)}'" if model else 'NULL'},
                {f"'{_escape(provider)}'" if provider else 'NULL'},
                0,
                0,
                '{_escape(meta_json)}'
            )
        ...
        logger.info(f"Created session {session_id} in project {proj_id}")
        return session_id
    ...
    def add_message(
        ...
        result = self._portal._db.query(f"""
        ...
        # Insert message
        self._portal._db.execute_command(f"""
            INSERT INTO _ci_session_messages (
                message_id, session_id, project_id, role, content,
                token_count, sequence_number, metadata
            ) VALUES (
                '{message_id}',
                '{session_id}',
                '{proj_id}',
        ...
        self._portal._db.execute_command(f"""
            ...
            SELECT
                session_id, project_id, title, description, model, provider,
                message_count + 1, token_count + {token_count}, metadata
            FROM _ci_sessions
            WHERE session_id = '{session_id}'
        """)
    ...
    def list_sessions(
        ...
        where_clauses = []
        if proj_id:
            where_clauses.append(f"project_id = '{proj_id}'")
    ...
    def archive_session(self, session_id: str) -> None:
        ...
        self._portal._db.execute_command(f"""
            INSERT INTO _ci_sessions (
                session_id, project_id, title, description, model, provider,
                message_count, token_count, archived_at, metadata
            )
            SELECT
                session_id, project_id, title, description, model, provider,
                message_count, token_count, now64(3), metadata
            FROM _ci_sessions
            WHERE session_id = '{session_id}'
        """)
    ...
    def create_todo(
        ...
        Args:
            ...
            status: Status (open, in_progress, completed, blocked)
        ...
        todo_id = str(uuid4())
        meta_json = json.dumps(metadata or {})
        tags_array = _format_value(tags or [])

        self._portal._db.execute_command(f"""
            INSERT INTO _ci_todos (
                todo_id, project_id, title, description, status, priority,
                file_path, line_number, assigned_to, due_date, tags, metadata
            ) VALUES (
                '{todo_id}',
                '{proj_id}',
                '{_escape(title)}',
                ...
                '{_escape(status)}',
                '{_escape(priority)}',
                {f"'{_escape(file_path)}'" if file_path else 'NULL'},
                {line_number if line_number else 'NULL'},
                {f"'{_escape(assigned_to)}'" if assigned_to else 'NULL'},
        ...
        logger.info(f"Created todo {todo_id} in project {proj_id}")
        return todo_id
    ...
    def update_todo(
        ...
        new_assigned_to = assigned_to if assigned_to is not None else current.get("assigned_to")
        new_due_date = due_date if due_date is not None else current.get("due_date")
        new_tags = tags if tags is not None else current.get("tags", [])

        # Set completed_at if status changed to completed
        completed_at = "now64(3)" if new_status == "completed" and current["status"] != "completed" else (
            f"'{current['completed_at']}'" if current.get("completed_at") else "NULL"
        )

        tags_array = _format_value(new_tags)

        # Insert updated record
        self._portal._db.execute_command(f"""
            INSERT INTO _ci_todos (
                todo_id, project_id, title, description, status, priority,
                file_path, line_number, assigned_to, completed_at, due_date,
                tags, metadata, created_at
            ) VALUES (
                '{todo_id}',
                '{current["project_id"]}',
                '{_escape(current["title"])}',
                {f"'{_escape(new_description)}'" if new_description else 'NULL'},
                '{_escape(new_status)}',
                '{_escape(new_priority)}',
                {f"'{_escape(current['file_path'])}'" if current.get('file_path') else 'NULL'},
                {current['line_number'] if current.get('line_number') else 'NULL'},
                {f"'{_escape(new_assigned_to)}'" if new_assigned_to else 'NULL'},
                {completed_at},
                {f"'{new_due_date}'" if new_due_date else 'NULL'},
                {tags_array},
                '{_escape(current.get("metadata", "{}") if isinstance(current.get("metadata"), str) else json.dumps(current.get("metadata", {})))}',
                '{current["created_at"]}'
            )
        """)
    ...
    def list_todos(
        ...
        if priority:
            where_clauses.append(f"priority = '{_escape(priority)}'")
        if assigned_to:
            where_clauses.append(f"assigned_to = '{_escape(assigned_to)}'")
        if file_path:
            where_clauses.append(f"file_path = '{_escape(file_path)}'")
    ...
    def delete_todo(self, todo_id: str) -> None:
        ...
        self.ensure_tables()
        self._portal._db.execute_command(
            f"ALTER TABLE _ci_todos DELETE WHERE todo_id = '{todo_id}'"
        )
